...
5.  Configure the gate map on the BlynkApp.  This will be a critical step if you have gates to open between your machine driven gate and the dust collector.   It's very simple, each position in the 0-1 map corresponds to an index of a gate.   If there is a 1, the gate should be opened, if closed, a 0.   Save the new name, tool name, gate map to EEPROM, and restart the device.    You're ready to go.
